---
name: sharek
description: Sharek is a tool to schedule social media and chat posts to 28+ channels X, LinkedIn, LinkedIn Page, Reddit, Instagram, Facebook Page, Threads, YouTube, Google My Business, TikTok, Pinterest, Dribbble, Discord, Slack, Kick, Twitch, Mastodon, Bluesky, Lemmy, Farcaster, Telegram, Nostr, VK, Medium, Dev.to, Hashnode, WordPress, ListMonk
homepage: https://github.com/sharekhq/sharek-agent#readme
metadata: {"openclaw":{"emoji":"🌎","requires":{"bins":[],"env":["SHAREK_API_URL"]}}}
---
...
**Rule 2 — Every file passed to `-m` (or to `image`/media fields in JSON mode) MUST first go through `sharek upload`.** Raw filesystem paths (`image.jpg`, `video.mp4`) and external URLs (`https://example.com/...`) are **NOT** accepted by the publishing pipeline. TikTok, Instagram, YouTube, and most other providers reject anything that isn't a Sharek-verified URL. Always:

```bash
RESULT=$(sharek upload <file>)
URL=$(echo "$RESULT" | jq -r '.path')
sharek posts:create ... -m "$URL" ...
```

If you see `-m "something.jpg"` anywhere below, treat it as shorthand for "the `.path` you got back from `sharek upload something.jpg`" — never a raw local file.

---

## ⚠️ Authentication Required

**You MUST authenticate before running any Sharek CLI command.** All commands will fail without valid credentials.
...
The fundamental pattern for using Sharek CLI:
...
# 3. Fetch (if needed)
sharek integrations:trigger <integration-id> <method> -d '{"key":"value"}'

# 4. Prepare
sharek upload image.jpg

# 5. Post
sharek posts:create -c "Content" -m "image.jpg" -i "<integration-id>"

# 6. Analyze
sharek analytics:platform <integration-id> -d 30
sharek analytics:post <post-id> -d 7
...
**Optional custom API URL (defaults to https://dash.sharek.app/api):**
```bash
export SHAREK_API_URL=https://custom-api-url.com
```
...
sharek integrations:list --group <group-id>
...
sharek integrations:trigger <integration-id> <method-name> -d '{"param":"value"}'
```
...
IMG1=$(sharek upload img1.jpg | jq -r '.path')
IMG2=$(sharek upload img2.jpg | jq -r '.path')
sharek posts:create -c "Content" -m "$IMG1,$IMG2" -s "2024-12-31T12:00:00Z" -i "integration-id"

# Post with comments (each with own media — every file uploaded first)
MAIN=$(sharek upload main.jpg | jq -r '.path')
C1=$(sharek upload comment1.jpg | jq -r '.path')
C2A=$(sharek upload comment2.jpg | jq -r '.path')
C2B=$(sharek upload comment3.jpg | jq -r '.path')
sharek posts:create \
  -c "Main post" -m "$MAIN" \
...
sharek posts:status <post-id> --status draft     # Move back to draft, terminates any running publish workflow
sharek posts:status <post-id> --status schedule  # Promote a draft into the publishing queue (uses the post's stored date)
```
...
**⚠️ IMPORTANT:** Always upload files to Sharek before using them in posts. Many platforms (TikTok, Instagram, YouTube) **require verified URLs** and will reject external links.
...
sharek posts:create -c "Content" -s "2024-12-31T12:00:00Z" -m "$VIDEO_PATH" -i "tiktok-id"
```
...
```bash
YOUTUBE_ID=$(sharek integrations:list | jq -r '.[] | select(.identifier=="youtube") | .id')
PLAYLISTS=$(sharek integrations:trigger "$YOUTUBE_ID" getPlaylists)
...
```bash
LINKEDIN_ID=$(sharek integrations:list | jq -r '.[] | select(.identifier=="linkedin") | .id')
COMPANIES=$(sharek integrations:trigger "$LINKEDIN_ID" getCompanies)
...
VIDEO_RESULT=$(sharek upload video.mp4)
VIDEO_PATH=$(echo "$VIDEO_RESULT" | jq -r '.path')

THUMB_RESULT=$(sharek upload thumbnail.jpg)
...
INTRO=$(sharek upload intro.jpg | jq -r '.path')
P1=$(sharek upload point1.jpg | jq -r '.path')
P2=$(sharek upload point2.jpg | jq -r '.path')
OUTRO=$(sharek upload outro.jpg | jq -r '.path')
      ...
      "post": [
        {
          ...
          "image": ["<URL returned by `sharek upload twitter-image.jpg`>"]
        }
      ]
    },
    {
      ...
      "post": [
        {
          ...
          "image": ["<URL returned by `sharek upload linkedin-image.jpg`>"]
        }
      ]
    }
  ]
...
sharek posts:create --json campaign.json
```
...
SETTINGS_JSON=$(sharek integrations:settings "$INTEGRATION_ID")
...
for i in "${!DATES[@]}"; do
  ...
  IMG=$(sharek upload "post-${i}.jpg" | jq -r '.path')
  sharek posts:create \
    -c "${CONTENT[$i]}" \
    -s "${DATES[$i]}" \
    -i "twitter-id" \
...
for attempt in $(seq 1 $MAX_RETRIES); do
  if sharek posts:create -c "$CONTENT" -s "$DATE" -i "$INTEGRATION_ID"; then
...
```bash
sharek posts:create -c "Content" -s "2024-12-31T12:00:00Z" --settings '{"subreddit":[...]}' -i "reddit-id"
...
I1=$(sharek upload image1.jpg | jq -r '.path')
I2=$(sharek upload image2.jpg | jq -r '.path')
CI=$(sharek upload comment-img.jpg | jq -r '.path')
A1=$(sharek upload another.jpg | jq -r '.path')
A2=$(sharek upload more.jpg | jq -r '.path')
...
Internally creates (note: every URL is a Sharek-uploaded `.path`, not a raw filename):
```json
{
  "posts": [{
    "value": [
...
```json
{
  "path": "https://dash.sharek.app/uploads/abc123.jpg",
  "size": 123456,
  "type": "image/jpeg"
}
```
...
```bash
RESULT=$(sharek upload image.jpg)
PATH=$(echo "$RESULT" | jq -r '.path')
sharek posts:create -c "Content" -s "2024-12-31T12:00:00Z" -m "$PATH" -i "integration-id"
```
...
```bash
sharek posts:create -c "Content" -m "img.jpg" -i "twitter-id"
```
...
```bash
# Personal post
sharek posts:create -c "Content" -s "2024-12-31T12:00:00Z" -i "linkedin-id"
...
sharek integrations:list --group <group-id>        # Get integration IDs in a group
sharek integrations:groups                         # List groups (customers)
sharek integrations:settings <id>                  # Get settings schema
sharek integrations:trigger <id> <method> -d '{}'  # Fetch dynamic data
...
sharek posts:create -c "text" -m "$(sharek upload img.jpg | jq -r '.path')" -s "2024-12-31T12:00:00Z" -i "id"  # With media (upload first — Rule 2)
