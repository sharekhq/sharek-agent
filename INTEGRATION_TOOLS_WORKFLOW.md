...
```bash
sharek integrations:settings reddit-abc123
```
...
```bash
sharek integrations:trigger reddit-abc123 getFlairs -d '{"subreddit":"programming"}'
```
...
```bash
sharek integrations:settings youtube-123
```
...
```bash
sharek integrations:trigger youtube-123 getPlaylists
```
...
```bash
sharek integrations:settings linkedin-123
```
...
```bash
sharek integrations:trigger linkedin-123 getCompanies
```
...
```bash
# No parameters
sharek integrations:trigger <integration-id> <methodName>

# With parameters
sharek integrations:trigger <integration-id> <methodName> -d '{"key":"value"}'
```
...
SETTINGS=$(sharek integrations:settings "$INTEGRATION_ID")
...
for METHOD in $TOOLS; do
  RESULT=$(sharek integrations:trigger "$INTEGRATION_ID" "$METHOD" -d '{}')
...
```bash
sharek integrations:trigger reddit-123 invalidMethod
...
```bash
sharek integrations:trigger reddit-123 getFlairs
...
```bash
sharek integrations:trigger invalid-id getFlairs
...
FLAIRS=$(sharek integrations:trigger $INTEGRATION_ID getFlairs -d '{"subreddit":"programming"}')
